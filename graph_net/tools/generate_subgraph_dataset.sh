...
DECOMPOSE_WORKSPACE=/work/graphnet_test_workspace/subgraph_dataset_test100_20251228/decomposed_${OP_RANGE}ops
...
function set_output_dirs() {
    ...
    DIMENSION_GENERALIZED_CLUSTER_DIR=$DIMENSION_GENERALIZED_OUTPUT_DIR/$cluster_name
    RANGE_DECOMPOSE_OUTPUT_DIR=$DECOMPOSE_WORKSPACE/03_range_decomposed_samples/$cluster_name
    GRAPH_VAR_RENAME_OUTPUT_DIR=$DECOMPOSE_WORKSPACE/04_renamed_subgraphs/$cluster_name
    DEDUPLICATED_OUTPUT_DIR=$DECOMPOSE_WORKSPACE/05_deduplicated_subgraphs/$cluster_name
    DEVICE_REWRITED_OUTPUT_DIR=$DECOMPOSE_WORKSPACE/06_device_rewrited_subgraphs/$cluster_name
    CUMSUM_NUM_KERNELS_DIR=$DECOMPOSE_WORKSPACE/07_cumsum_num_kernels/$cluster_name
    FUSIBLE_SUBGRAPH_RANGES_DIR=$DECOMPOSE_WORKSPACE/08_fusible_subgraph_ranges/$cluster_name
    FUSIBLE_SUBGRAPH_SAMPLES_DIR=$DECOMPOSE_WORKSPACE/09_fusible_subgraphs/$cluster_name
    RENAMED_FUSIBLE_SUBGRAPH_DIR=$DECOMPOSE_WORKSPACE/10_renamed_fusible_subgraphs/$cluster_name
    DEDUPLICATED_FUSIBLE_SUBGRAPH_DIR=$DECOMPOSE_WORKSPACE/11_deduplicated_fusible_subgraphs/$cluster_name
    UNITTESTS_OUTPUT_DIR=$DECOMPOSE_WORKSPACE/12_kernelbench_unittests/$cluster_name

    dimension_cluster_model_list=${DECOMPOSE_WORKSPACE}/dimension_generalized_samples_list_${cluster_name}.txt
    range_decomposed_subgraph_list=${DECOMPOSE_WORKSPACE}/range_decomposed_subgraph_sample_list_${cluster_name}.txt
    deduplicated_subgraph_list=${DECOMPOSE_WORKSPACE}/deduplicated_subgraph_sample_list_${cluster_name}.txt
    device_rewrited_subgraph_list=${DECOMPOSE_WORKSPACE}/device_rewrited_subgraph_sample_list_${cluster_name}.txt
    fusible_subgraph_list=${DECOMPOSE_WORKSPACE}/fusible_subgraph_sample_list_${cluster_name}.txt
    deduplicated_fusible_subgraphs_list=${DECOMPOSE_WORKSPACE}/deduplicated_fusible_subgraph_sample_list_${cluster_name}.txt
}
...
function dimension_generalization() {
    echo ">>> [3] Dimension generalization for samples in ${model_list}."
    echo ">>>"
    python3 -m graph_net.model_path_handler \
        --model-path-list $model_list \
        --handler-config=$(base64 -w 0 <<EOF
{
    "handler_path": "$GRAPH_NET_ROOT/graph_net/dimension_generalizer.py",
...
EOF
    ...
    python3 restructure.py \
        --cluster-name "dim_cluster" \
        --source-dir $DIMENSION_GENERALIZED_OUTPUT_DIR \
        --target-dir $DIMENSION_GENERALIZED_OUTPUT_DIR
}

function range_decompose() {
    echo ">>> [4] Decompose according to split_results.json for samples in ${dimension_cluster_model_list}."
    echo ">>>"
    python3 -m graph_net.model_path_handler \
        --model-path-list "$dimension_cluster_model_list" \
        --handler-config=$(base64 -w 0 <<EOF
{
    ...
    "handler_config": {
        "resume": ${RESUME},
        "model_path_prefix": "$DIMENSION_GENERALIZED_CLUSTER_DIR",
...
main() {
    ...
    dimension_generalization 2>&1 | tee ${DECOMPOSE_WORKSPACE}/log_dimension_generalization_${suffix}.txt

    set_output_dirs "dim_cluster_0"
    generate_subgraph_list ${DIMENSION_GENERALIZED_CLUSTER_DIR} ${dimension_cluster_model_list}
